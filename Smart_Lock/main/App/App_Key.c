...
static void App_Key_AddAdmin(void)
{
    ...
    // 判断是否有管理员
    // 管理员已满
    esp_err_t err = Dri_NVS_FindKey("admain");
    if (err == ESP_OK)
    {
        sayWithoutInt();
        sayAdmIntull(); // 管理员已满
    ...
    {
        sayAddSucc(); // 添加成功
        Dri_NVS_AddKeyValue("admain", (char *)input2);
        // 清空缓冲区
        App_Key_ClearBuff();
    }
    else
...
{
    // 1. 判断管理员是否存在
    if (Dri_NVS_FindKey("admain") != ESP_OK)
    {
        sayWithoutInt();
        sayInvalid();
        return false;
    ...
    Dri_NVS_GetKey("admain", (char *)input2, &input2_size);
 ...
 *
 */
static void App_Key_DeleteAdmain(void)
{
    // 1. 语音播报删除管理员
    sayWithoutInt();
    sayDelAdmin();
